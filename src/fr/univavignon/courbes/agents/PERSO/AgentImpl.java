package fr.univavignon.courbes.agents.PERSO;
...
public class AgentImpl extends Agent
{
			...
			// si la position est visible par le serpent de l'agent
				...
				{	// on calcule la distance entre cette position et la tête du serpent de l'agent
					// si la position est plus proche que le plus proche obstacle connu : on met à jour
						if(dist<result[0] && dist<220*agent.movingSpeed*agent.headRadius)
						{	
	...
	{	checkInterruption();	// on doit tester l'interruption au début de chaque méthode
		...
		{	checkInterruption();	// une boucle, donc un autre test d'interruption
			...
			if(getPlayerId() != snake.playerId ){
				...
				{	// on calcule la distance entre cette position et la tête du serpent de l'agent
					// si la position est plus proche que le plus proche obstacle connu : on met à jour
						if(dist<result[0] && dist > 5 && dist<220*agent.movingSpeed*agent.headRadius)
						{	
							result[0] = dist;	// mise à jour de la distance
							result[1] = angle;	// mise à jour de l'angle
						}	
	...
	public boolean checkIfSafe(){
		...
		if(tab[0] < 220*agent.movingSpeed*agent.headRadius){
			return false;
		}
		else return true;
		
		/*int x = agent.currentX;
		int y = agent.currentY;
		Board board = getBoard();
		
		// tests bordures
		for(float i = x-220*agent.movingSpeed*agent.headRadius ; i < x+220*agent.movingSpeed*agent.headRadius ; i++){
			checkInterruption();	// on doit tester l'interruption au début de chaque boucle
			for(float j = y-220*agent.movingSpeed*agent.headRadius;j<y+180*agent.movingSpeed*agent.headRadius;j++){
				if(i<=0 ||  i>=board.width || j<=0 || j>=board.height)return false;
			}
			
		}
		
		Snake[] tab = board.snakes;
		...
		for(Snake snake : tab){
			...
			{	checkInterruption();	// une boucle, donc un autre test d'interruption
				...
				double angle = Math.atan2(position.y-agent.currentY, position.x-agent.currentX);
				if(angle<0)
					angle = angle + 2*Math.PI;
				...
				if(isInSight(angle)){
					...
					if(getPlayerId() != snake.playerId){
						if(dist < 220*agent.movingSpeed*agent.headRadius){
							return false;
						}
					}
					else{
						if(dist < 220*agent.movingSpeed*agent.headRadius){
							System.out.println("me danger");
							return false;
						}//TODO : ici pour la gestion de l'agent sur lui meme
					}
				}		
			}
		}
...
public static int userFast(ItemInstance object, Snake enemy)
{
	...
	/*if(this.pluscourtchemin(object.x,object.y)<enemy.pluscourtchemin(object.x,object.y))
	{
		if(this.pluscourtchemin(object.x,object.y)<10 && this.pluscourtchemin(enemy.x,enemy.y)+enemy.movingSpeed*object.type.duration-this.movingSpeed*Constants.MOVING_SPEED_COEFF*object.type.duration<5)
		{
			return 100;
		}
		else
